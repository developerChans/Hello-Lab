...
const LabSideBar = ({ data, updateTab, updateCategory }) => {
  ...
  const onResearchHandler = (section) => {
    ...
    window.location.href = `/lab/${data.lab.id}/research/${section.id}/process`;
  };
    ...
    <div className="lab-sidebar">
      ...
      <div className="lab-sidebar-tabs">
        {tabs.map((section, index) => (
          <div
            key={index}
            className="lab-sidebar-tab"
            onClick={() => onTabHandler(section)}
            type="button"
          >
            <span className="">{section.tab}</span>
          </div>
        ))}
        <div className="lab-sidebar-research">
          <div
            className="lab-sidebar-research-dropdown"
            type="button"
          ...
          <div ref={researchTabs} className="lab-sidebar-research-tabs">
            {researchList.map((section, index) => (
              <div
                key={index}
                className="lab-sidebar-research-tab"
                type="button"
                onClick={() => onResearchHandler(section)}
              >
                <span className="">{section.tab}</span>
              </div>
            ))}
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    updateCategory: (category) =>
      dispatch(actionCreators.updateCategory(category)),
  };
};
